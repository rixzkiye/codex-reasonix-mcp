...
describe('verification subprocess isolation', () => {
  it.skipIf(!sandboxStatus.available)(
    'does not forward provider credentials to contract commands',
    async () => {
      ...
      try {
            ...
            verification: [
              {
                ...
                argv: [
                  process.execPath,
                  '-e',
                  'if (process.env.DEEPSEEK_API_KEY) process.exit(17)',
                ],
                cwd: '.',
                timeout_seconds: 30,
                proves: ['ac_result'],
        ...
        const evidence = await runAllVerification(task, store, loadConfig({}));
